...
{
  Place_data <- tibble(place, date, events, new_events)
  ...
  for (i in 1:Place_END) {
    ...
    #Statei$Y_Max <- pmax( Statei$Y_Max, Statei$Y_Min)
    ...
    Date_P2 <- min(Statei$date[Statei$Criteria1==1], na.rm = TRUE)
    ...
    Day_P2 <- min(Statei$Days_N[Statei$Criteria1==1], na.rm = TRUE)
    ...
    if (New_Phase) {
      ...
      while (p < nrow(Statei)) {
        ...
        #        New_Phase <- is.finite(last(Statei$Day_PC) )
