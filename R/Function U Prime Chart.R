...
U_Prime <- function(Uchart_Data) {
    ...
    while (j <= Days_Tot) {
      ...
      if (j < 7 | New_Phase) {
        ...
        Uchart_Data$Sigma_i <- sqrt(Uchart_Data$CL/Uchart_Data$N)
        Uchart_Data$Zi <- (Uchart_Data$Dot - Uchart_Data$CL)/Uchart_Data$Sigma_i

        
        Uchart_Data$MR <- abs( Uchart_Data$Zi - lag(Uchart_Data$Zi) )
        
        
        MR_UCL_2 <- 3.27*mean(Uchart_Data$MR[i:m], na.rm = TRUE)
        
        Uchart_Data$Screened_MR <- if_else(Uchart_Data$MR <= MR_UCL_2, 1, 0)
        
        
        SigmaZ <- ( mean(Uchart_Data$MR[i:m], na.rm = TRUE) ) /  1.128
        
        
        
        Uchart_Data$LL <- Uchart_Data$CL - 3*SigmaZ*sqrt(Uchart_Data$CL/Uchart_Data$N)
        Uchart_Data$UL <- Uchart_Data$CL + 3*SigmaZ*sqrt(Uchart_Data$CL/Uchart_Data$N)
